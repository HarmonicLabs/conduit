...
var Script = /** @class */ (function () {
    function Script(script, cborRef) {
        if (cborRef === void 0) { cborRef = undefined; }
        this.cborRef = cborRef;
        // console.log("type: ", this.type);
        var scriptType = script.scriptType, bytes = script.bytes;
        if (!(scriptType === ScriptType.NativeScript ||
        ...
        if (this.type !== ScriptType.NativeScript) {
            this.cbor = cbor_1.Cbor.encode(new cbor_1.CborBytes(cbor_1.Cbor.encode(new cbor_1.CborBytes(Uint8Array.prototype.slice.call(bytes))).toBuffer()));
        }
        ;
        this.cborRef = cborRef !== null && cborRef !== void 0 ? cborRef : (0, getSubCborRef_1.subCborRefOrUndef)(script);
    }
    Object.defineProperty(Script.prototype, "hash", {
        get: function () {
            if (this._hash !== undefined && this._hash instanceof hashes_1.Hash28)
                return this._hash;
            var scriptDataToBeHashed = [];
            if (this.type === ScriptType.NativeScript)
                scriptDataToBeHashed = [0x00].concat(Array.from(this.bytes));
            else {
                var singleCbor = Array.from(cbor_1.Cbor.encode(new cbor_1.CborBytes(this.bytes)).toBuffer());
            ...
            this._hash = new hashes_1.Hash28(Uint8Array.from((0, crypto_1.blake2b_224)(scriptDataToBeHashed)));
            return this._hash;
        },
        enumerable: false,
        configurable: true
    ...
    Script.fromJson = function (json) {
        ...
        if (t !== ScriptType.NativeScript) {
            return new Script({
                scriptType: t,
                bytes: (0, uint8array_utils_1.fromHex)(json.cborHex)
            });
        }
        return new Script({
    ...
    **/
    Script.prototype.toCborBytes = function () {
    ...
    **/
    ...
    Script.fromCborObj = function (cObj, defType) {
        ...
        if (cObj instanceof cbor_1.CborBytes) {
            return new Script({
                scriptType: defType,
                bytes: cbor_1.Cbor.encode(cObj).toBuffer()
            });
        }
        ...
        if (t === ScriptType.NativeScript)
            return new Script({
                scriptType: t,
                bytes: cbor_1.Cbor.encode(cObj.array[1]).toBuffer()
            });
