"use strict";
var __assign = (this && this.__assign) || function () {
    __assign = Object.assign || function(t) {
        for (var s, i = 1, n = arguments.length; i < n; i++) {
            s = arguments[i];
            for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p))
                t[p] = s[p];
        }
        return t;
    };
    return __assign.apply(this, arguments);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.DRepKeyHash = exports.isIDRepKeyHash = void 0;
var cbor_1 = require("@harmoniclabs/cbor");
var credentials_1 = require("../../credentials/index.js");
var hashes_1 = require("../../hashes/index.js");
var roDescr_1 = require("../../utils/roDescr.js");
var DRepType_1 = require("./DRepType.js");
var obj_utils_1 = require("@harmoniclabs/obj-utils");
var plutus_data_1 = require("@harmoniclabs/plutus-data");
var defaultToDataVersion_1 = require("../../toData/defaultToDataVersion.js");
var getSubCborRef_1 = require("../../utils/getSubCborRef.js");
function isIDRepKeyHash(stuff) {
    return (0, obj_utils_1.isObject)(stuff) && (0, hashes_1.canBeHash28)(stuff.hash);
}
exports.isIDRepKeyHash = isIDRepKeyHash;
var DRepKeyHash = /** @class */ (function () {
    function DRepKeyHash(_a, cborRef) {
        var hash = _a.hash;
        if (cborRef === void 0) { cborRef = undefined; }
        this.cborRef = cborRef;
        Object.defineProperties(this, {
            drepType: __assign({ value: DRepType_1.DRepType.KeyHash }, roDescr_1.roDescr),
            hash: __assign({ value: new credentials_1.PubKeyHash(hash) }, roDescr_1.roDescr)
        });
    }
    DRepKeyHash.prototype.toData = function (version) {
        version = (0, defaultToDataVersion_1.definitelyToDataVersion)(version);
        if (version === "v1" || version === "v2")
            throw new Error("DRep only supported after v3");
        return new plutus_data_1.DataConstr(0, // PDRep.DRep
        [credentials_1.Credential.keyHash(this.hash).toData(version)]);
    };
    DRepKeyHash.prototype.toCborBytes = function () {
        if (this.cborRef instanceof cbor_1.SubCborRef)
            return this.cborRef.toBuffer();
        return this.toCbor().toBuffer();
    };
    DRepKeyHash.prototype.toCbor = function () {
        if (this.cborRef instanceof cbor_1.SubCborRef) {
            // TODO: validate cbor structure
            // we assume correctness here
            return new cbor_1.CborString(this.cborRef.toBuffer());
        }
        return cbor_1.Cbor.encode(this.toCborObj());
    };
    DRepKeyHash.prototype.toCborObj = function () {
        return new cbor_1.CborArray([
            new cbor_1.CborUInt(this.drepType),
            this.hash.toCborObj()
        ]);
    };
    DRepKeyHash.fromCborObj = function (cbor) {
        if (!(cbor instanceof cbor_1.CborArray &&
            cbor.array.length >= 2 &&
            cbor.array[0] instanceof cbor_1.CborUInt &&
            Number(cbor.array[0].num) === DRepType_1.DRepType.KeyHash))
            throw new Error("Invalid cbor for 'DRepKeyHash'");
        return new DRepKeyHash({
            hash: hashes_1.Hash28.fromCborObj(cbor.array[1])
        }, (0, getSubCborRef_1.getSubCborRef)(cbor));
    };
    DRepKeyHash.prototype.toJSON = function () { return this.toJson(); };
    DRepKeyHash.prototype.toJson = function () {
        return {
            drepType: (0, DRepType_1.drepTypeToString)(this.drepType),
            hash: this.hash.toString()
        };
    };
    return DRepKeyHash;
}());
exports.DRepKeyHash = DRepKeyHash;
